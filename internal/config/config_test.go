...
func TestLoad_MissingMinIOBucketIsRejectedAndNamed(t *testing.T) {
	setRequired(t)
	...
	if missing.Key != "MINIO_BUCKET" {
		t.Fatalf("expected the error to name MINIO_BUCKET, got %q", missing.Key)
	}
}

func TestLoad_RejectsMinIOEndpointWithScheme(t *testing.T) {
	setRequired(t)
...
func TestLoad_RejectsInvalidMinIOUseSSL(t *testing.T) {
	setRequired(t)
	t.Setenv("MINIO_USE_SSL", "yes-please")
	...
	if invalid.Key != "MINIO_USE_SSL" {
		t.Fatalf("expected the error to name MINIO_USE_SSL, got %q", invalid.Key)
	}
}
